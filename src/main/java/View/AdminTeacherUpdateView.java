...
public class AdminTeacherUpdateView extends JFrame{
    ...
    private void button1(ActionEvent e) {
        ...
        if (!id.equals(id1) || id.isEmpty()) {
            ...
            JOptionPane.showMessageDialog(contentPane, "工号禁止修改！", "系统提示", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (!username.equals(username1) || username.isEmpty()) {
            this.TeacherAddView.dispose();
            JOptionPane.showMessageDialog(contentPane, "账号禁止修改！", "系统提示", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        if (!teacher.getUsername().equals(username)) {
            Teacher exist = teacherDao.getByUsername(username);
            if (exist != null) {
        ...
        boolean flag = teacherDao.update(teacher);
        if (flag) {
    ...
    public AdminTeacherUpdateView( final Integer ids, final AdminManagement frame) {
        this.ids = ids;
        this.frame = frame;
        ...
        //======== TeacherAddView ========
        {
            TeacherAddView.setTitle("辅导员信息编辑");
        ...
        try {
            Teacher teacher = teacherDao.getById(ids);
            textField1.setText(teacher.getId());
            id1 = teacher.getId();

            textField2.setText(teacher.getUsername());
            username1 = teacher.getUsername();

            textField3.setText(teacher.getPassword());
            textField4.setText(teacher.getNickname());
            textField5.setText(teacher.getBirthday());
            textField6.setText(teacher.getTelephone());
            textField7.setText(teacher.getEmail());
            textField8.setText(teacher.getProfession());
            if (teacher.getSex() == 0) {
                radioButton1.setSelected(true);
            } else {
                radioButton2.setSelected(true);

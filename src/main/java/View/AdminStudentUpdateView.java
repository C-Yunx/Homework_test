 ...
 * @author 13425
 */
public class AdminStudentUpdateView extends JFrame{
...
//        if (!id.equals(id1) || id.isEmpty()) {
//            this.StudentAddView.dispose();
//            JOptionPane.showMessageDialog(contentPane, "学号禁止修改", "系统提示", JOptionPane.WARNING_MESSAGE);
//            return;
//        }
//        if (!username.equals(username1) || username.isEmpty()) {
//            this.StudentAddView.dispose();
//            JOptionPane.showMessageDialog(contentPane, "登录账号禁止修改", "系统提示", JOptionPane.WARNING_MESSAGE);
//            return;
//        }
        int sex = 0;
        //获取性别
    ...
    public AdminStudentUpdateView(final Integer ids, final AdminManagement frame) {
        this.ids = ids;
        this.frame = frame;
        ...
        //======== StudentAddView ========
        {
            StudentAddView.setTitle("学生信息编辑");
        ...
        try {
            Student student = studentDao.getById(ids);
            textField1.setText(student.getId());
//            id1 = student.getId();

            textField2.setText(student.getClassNumber());

            textField3.setText(student.getUsername());
//            username1 = student.getUsername();

            textField4.setText(student.getPassword());
            textField5.setText(student.getNickname());
            textField6.setText(student.getBirthday());
            textField7.setText(student.getTelephone());
            textField8.setText(student.getEmail());
            textField9.setText(student.getProfession());
            textField10.setText(String.valueOf(student.getProClassGrade()));
            textField11.setText(String.valueOf(student.getPeClassGrade()));
            textField12.setText(String.valueOf(student.getCultureClassGrade()));
            textField13.setText(String.valueOf(student.getComprehensiveTestGrade()));
            if (student.getSex() == 0) {
                radioButton1.setSelected(true);
            } else {
                radioButton2.setSelected(true);
            }
            if (student.getIfPool() == 0) {
                radioButton3.setSelected(true);
            } else {
                radioButton4.setSelected(true);
            }
            if (student.getIfViolationOfDiscipline() == 0) {
                radioButton5.setSelected(true);
            } else {
                radioButton6.setSelected(true);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(contentPane, "数据错误", "系统提示", JOptionPane.WARNING_MESSAGE);
        }
    }
